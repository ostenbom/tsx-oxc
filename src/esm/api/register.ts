...
export const register: Register = (options) => {
	if (!module.register) {
		throw new Error(
	...
	if (!cjsInteropApplied) {
		...
		module._resolveFilename = (request, ...restOfArgs) =>
			_resolveFilename(interopCjsExports(request), ...restOfArgs);
